...
if ( !function_exists( 'whpdf_get_prop' ) ) {
    /**
     *
     */
    function whpdf_get_prop( $object, $key, $single = true, $context = 'view' ) {

        $prop_map   = whpdf_return_new_attribute_map();
        ...
        if ( $is_wc_data ) {
            ...
            if ( isset( $object->$key ) ) {
                return $object->$key;
            } elseif ( whpdf_wc_check_post_columns( $key ) ) {
...
if ( !function_exists( 'whpdf_set_prop' ) ) {
    /**
     *
     */
    function whpdf_set_prop( $object, $arg1, $arg2 = false ) {
        ...
        foreach ( $arg1 as $key => $value ) {
            if ( $is_wc_data ) {
                ...
                if ( whpdf_wc_check_post_columns( $key ) ) {
...
if ( !function_exists( 'whpdf_save_prop' ) ) {
    /**
     *
     */
    function whpdf_save_prop( $object, $arg1, $arg2 = false, $force_update = false ) {
        if ( !is_array( $arg1 ) ) {
            $arg1 = array(
                $arg1 => $arg2
            );
        ...
        foreach ( $arg1 as $key => $value ) {
            whpdf_set_prop( $object, $key, $value );

            if ( !$is_wc_data ) {

                if ( whpdf_wc_check_post_columns( $key ) ) {
                    whpdf_store_changes( $object->post, $key, $value );
                } else {
                    $object_id = is_callable( array( $object, 'get_id' ) ) ? $object->get_id() : $object->id;
...
if ( !function_exists( 'whpdf_delete_prop' ) ) {
    /**
     *
     */
    function whpdf_delete_prop( $object, $key, $value = '' ) {
        $prop_map   = whpdf_return_new_attribute_map();
        $is_wc_data = $object instanceof WC_Data;
        ...
        if ( $is_wc_data ) {
            ...
            if ( whpdf_wc_check_post_columns( $key ) && ( !$value || $object->post->$key == $value ) ) {
                whpdf_store_changes( $object->post, $key, '' );
            } else {
                $object_id = is_callable( array( $object, 'get_id' ) ) ? $object->get_id() : $object->id;
...
if ( !function_exists( 'whpdf_return_new_attribute_map' ) ) {
    function whpdf_return_new_attribute_map() {
        return array(
...
if ( !function_exists( 'whpdf_store_changes' ) ) {
    function whpdf_store_changes( $object, $key, $value = false ) {
...
if ( !function_exists( 'whpdf_send_changes_to_db' ) ) {
    function whpdf_send_changes_to_db() {
        global $changed_objects;
        ...
        if ( !empty( $changed_objects ) ) {
            foreach ( $changed_objects as $id => $data ) {
                if ( version_compare( WC()->version, '2.7.0', '>=' ) ) {
                    ...
                    whpdf_set_prop( $object, $data[ 'changes' ] );
                    $object->save();
                } else {
                    $data[ 'ID' ] = $id;
                    wp_update_post( $data );
...
if ( !function_exists( 'whpdf_get_orders' ) ) {
    /**
     *
     */
    function whpdf_get_orders( $args ) {
...
if ( !function_exists( 'whpdf_get_products' ) ) {
    /**
     *
     */
    function whpdf_get_products( $args ) {
        if ( version_compare( WC()->version, '2.7', '<' ) ) {
            $args[ 'fields' ] = 'objects';
            $posts            = get_posts( $args );
...
if ( !function_exists( 'whpdf_update_product_stock' ) ) {
    /**
     *
     */
    function whpdf_update_product_stock( $product, $stock_quantity = 1, $operation = 'set' ) {
...
if ( !function_exists( 'whpdf_wc_deprecated_filters' ) ) {
    /**
     *
     */
    function whpdf_wc_deprecated_filters() {
        return apply_filters( 'whpdf_wc_deprecated_filters', array(
...
if ( !function_exists( 'whpdf_fix_wc_deprecated_filters' ) ) {
    /**
     *
     */
    function whpdf_fix_wc_deprecated_filters() {

        if ( !version_compare( WC()->version, '2.7.0', '<' ) ) {
            return;
        }

        $deprecated_filters = whpdf_wc_deprecated_filters();
        foreach ( $deprecated_filters as $old => $new ) {
            add_filter( $old, 'whpdf_wc_deprecated_filter_mapping', 10, 100 );
        }
    }
}

if ( !function_exists( 'whpdf_wc_deprecated_filter_mapping' ) ) {
    /**
     *
     */
    function whpdf_wc_deprecated_filter_mapping() {
        $deprecated_filters = whpdf_wc_deprecated_filters();
...
if ( !function_exists( 'whpdf_wc_check_post_columns' ) ) {
    /**
     *
     */
    function whpdf_wc_check_post_columns( $key ) {
        $columns = array(
            'post_author',
            'post_date',
            'post_date_gmt',
...
if ( !function_exists( 'whpdf_get_order_id' ) ) {
    /**
     ...
     * @return mixed
     */
    function whpdf_get_order_id( $order ) {
        return whpdf_get_prop( $order, 'id' );
    }
}

if ( !function_exists( 'whpdf_get_product_id' ) ) {
    /**
     ...
     * @return mixed
     */
    function whpdf_get_product_id( $product ) {
        return whpdf_get_prop( $product, 'id' );
    }
}

if ( !function_exists( 'whpdf_get_base_product_id' ) ) {
    /**
     ...
     * @return mixed
     */
    function whpdf_get_base_product_id( $product ) {

        return $product instanceof WC_Data && $product->is_type( 'variation' ) ?
            whpdf_get_prop( $product, 'parent_id' ) :
            whpdf_get_prop( $product, 'id' );
    }
}

if ( !function_exists( 'whpdf_get_display_price' ) ) {
    /**
     * @param WC_Product $product
     * @param string     $price
     * @param int        $qty
     */
    function whpdf_get_display_price( $product, $price = '', $qty = 1 ) {
...
if ( !function_exists( 'whpdf_get_price_excluding_tax' ) ) {
    /**
     * @param WC_Product $product
     * @param int        $qty
     * @param string     $price
     ...
     * @return float|string
     */
    function whpdf_get_price_excluding_tax( $product, $qty = 1, $price = '' ) {
...
if ( !function_exists( 'whpdf_get_price_including_tax' ) ) {
    /**
     * @param WC_Product $product
     * @param int        $qty
     * @param string     $price
     ...
     * @return float|string
     */
    function whpdf_get_price_including_tax( $product, $qty = 1, $price = '' ) {
...
if ( !function_exists( 'whpdf_get_product_image_id' ) ) {
    /**
     ...
     */
    function whpdf_get_product_image_id( $product, $context = 'view' ) {
...
if ( !function_exists( 'whpdf_get_refund_amount' ) ) {
    /**
     ...
     * @return float
     */
    function whpdf_get_refund_amount( $refund, $context = 'view' ) {
...
if ( !function_exists( 'whpdf_set_refund_amount' ) ){
    /**
     * @param $refund \WC_Order_Refund
     * @param $amount float
     *
     * @return float
     */
    function whpdf_set_refund_amount( $refund, $amount ){
        $is_wc_data = $refund instanceof WC_Data;
...
if ( !function_exists( 'whpdf_get_refund_reason' ) ){
    /**
     * @param $refund \WC_Order_Refund
     * @param $amount float
     *
     * @return float
     */
    function whpdf_get_refund_reason( $refund ){
        $is_wc_data = $refund instanceof WC_Data;
...
if ( !function_exists( 'whpdf_add_select2_fields' ) ) {
    /**
     * Add select 2
     *
     * @param array $args
     */
    function whpdf_add_select2_fields( $args = array() ) {
        $default = array(
            'type'              => 'hidden',
            'class'             => '',
            'id'                => '',
...
if ( !function_exists( 'whpdf_product_visibility_meta' ) ) {
    function whpdf_product_visibility_meta( $args ) {
        if ( version_compare( WC()->version, '2.7.0', '<' ) ) {
...
if ( !function_exists( 'whpdf_datetime_to_timestamp' ) ) {

    /**
     *
     */
    function whpdf_datetime_to_timestamp( $date ) {
...
whpdf_fix_wc_deprecated_filters();
add_action( 'shutdown', 'whpdf_send_changes_to_db' );